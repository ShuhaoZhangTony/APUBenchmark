                  ...
                  OptionParser &op)
{
    ...
    // OpenCL doesn't support templated kernels, so we have to use macros
 //   string spMacros = "-DSINGLE_PRECISION";
 //   runTest_ext<float, float4, float4>
 //      ("MD-LJ", dev, ctx, queue, resultDB, op, spMacros);

    // If double precision is supported, run the DP test
    if (checkExtension(dev[0], "cl_khr_fp64"))
    {
        cout << "DP Supported\n";
        string dpMacros = "-DK_DOUBLE_PRECISION ";
        runTest_ext<double, double4, double4>
                ("MD-LJ-DP", dev, ctx, queue, resultDB, op, dpMacros);
    }
    else if (checkExtension(dev[0], "cl_amd_fp64"))
    {
        cout << "DP Supported\n";
        string dpMacros = "-DAMD_DOUBLE_PRECISION ";
        runTest_ext<double, double4, double4>
        ...
        string compileFlags)
{
    ...
    h_pos = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
            sizeof(posVecType)*nAtom, NULL, &err);
    CL_CHECK_ERROR(err);
    position = (posVecType*)clEnqueueMapBuffer(queue[1], h_pos, true,
            CL_MAP_READ|CL_MAP_WRITE, 0, sizeof(posVecType)*nAtom , 0,
            NULL, NULL, &err);
    CL_CHECK_ERROR(err);
    // Force
    h_force = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
            sizeof(forceVecType)*nAtom, NULL, &err);
    CL_CHECK_ERROR(err);
    force = (forceVecType*)clEnqueueMapBuffer(queue[1], h_force, true,
            CL_MAP_READ|CL_MAP_WRITE, 0, sizeof(forceVecType)*nAtom , 0,
            NULL, NULL, &err);
    CL_CHECK_ERROR(err);
    // Neighbor List
    h_neigh = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
            sizeof(int) * nAtom * maxNeighbors, NULL, &err);
    CL_CHECK_ERROR(err);
    neighborList = (int*)clEnqueueMapBuffer(queue[1], h_neigh, true,
            CL_MAP_READ|CL_MAP_WRITE, 0, sizeof(int) * nAtom * maxNeighbors, 0,
            NULL, NULL, &err);
    CL_CHECK_ERROR(err);

    // Allocate device memory
    cl_mem d_force = clCreateBuffer(ctx, CL_MEM_READ_WRITE| CL_MEM_ALLOC_HOST_PTR,
            nAtom * sizeof(forceVecType), NULL, &err);
    CL_CHECK_ERROR(err);
    cl_mem d_position = clCreateBuffer(ctx, CL_MEM_READ_WRITE| CL_MEM_ALLOC_HOST_PTR,
            nAtom * sizeof(posVecType), NULL, &err);
    CL_CHECK_ERROR(err);
    // Allocate device memory neighbor list
    cl_mem d_neighborList = clCreateBuffer(ctx, CL_MEM_READ_WRITE| CL_MEM_ALLOC_HOST_PTR,
            maxNeighbors * nAtom * sizeof(int), NULL, &err);
    CL_CHECK_ERROR(err);

    size_t maxGroupSize = std::min(getMaxWorkGroupSize(dev[1]),getMaxWorkGroupSize(dev[0]));
    if (maxGroupSize < 128)
    {
        cout << "MD requires a work group size of at least 128" << endl;
        // Add special values to the results database
    ...
    // Copy position to GPU
    Event evTransfer("h->d transfer");
    err = clEnqueueWriteBuffer(queue[1], d_position, true, 0,
            nAtom * sizeof(posVecType), position, 0, NULL,
            &evTransfer.CLEvent());
    CL_CHECK_ERROR(err);
    ...
    // Copy data to GPU
    err = clEnqueueWriteBuffer(queue[1], d_neighborList, true, 0,
            maxNeighbors * nAtom * sizeof(int), neighborList, 0, NULL,
            &evTransfer.CLEvent());
    CL_CHECK_ERROR(err);
    clFinish(queue[1]);
    ...
    err = clBuildProgram(prog, 0, NULL, compileFlags.c_str(), NULL, NULL);
...
//    Event evLJ("computeLJ");
	cl_event* evLJ = (cl_event*)malloc(sizeof(cl_event)*2);	
    // Warm up the kernel and check correctness
    int rt = clEnqueueNDRangeKernel_tony(queue, lj_kernel, 1, &globalSize,
            &localSize, evLJ);
	if(rt!=-1){
		FillTimingInfo(evLJ,rt);
	}
    err = clEnqueueReadBuffer(queue[1], d_force, true, 0,
            nAtom * sizeof(forceVecType), force, 0, NULL,
            &evTransfer.CLEvent());
    CL_CHECK_ERROR(err);
    err = clFinish(queue[1]);
    ...
    for (int i = 0; i < passes; i++)
    {
        ...
        {
            //Launch Kernels
            int rt = clEnqueueNDRangeKernel_tony(queue, lj_kernel, 1,
		...
		FillTimingInfo(evLJ,rt);
            ...
            // Collect timing info from events
            total_time += std::max(SubmitEndRuntime(0),SubmitEndRuntime(1));
        }
    ...
    // Clean up
    // Host memory
    err = clEnqueueUnmapMemObject(queue[1], h_pos,   position, 0, NULL, NULL);
    CL_CHECK_ERROR(err);
    err = clEnqueueUnmapMemObject(queue[1], h_force, force, 0, NULL, NULL);
    CL_CHECK_ERROR(err);
    err = clEnqueueUnmapMemObject(queue[1], h_neigh, neighborList, 0, NULL, NULL);
    CL_CHECK_ERROR(err);
    err = clReleaseMemObject(h_pos);
    CL_CHECK_ERROR(err);
    err = clReleaseMemObject(h_force);
