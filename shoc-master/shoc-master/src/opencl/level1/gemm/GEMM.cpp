...
template <class T>
void runTest_ext(const string& testName, cl_device_id* dev, cl_context ctx,
...
void
RunBenchmark_ext(cl_device_id* dev,
                  cl_context ctx,
                  cl_command_queue* queue,
                  ResultDatabase &resultDB,
                  OptionParser &op)
...
//            "-DSINGLE_PRECISION");
    ...
    {
        cout << "DP Supported\n";
        runTest_ext<double>("DGEMM", dev, ctx, queue, resultDB, op,
                "-DK_DOUBLE_PRECISION ");
    }
    else if (checkExtension(dev[0], "cl_amd_fp64") && checkExtension(dev[1], "cl_amd_fp64"))
    {
        cout << "DP Supported\n";
        runTest_ext<double>("DGEMM", dev, ctx, queue, resultDB, op,
                "-DAMD_DOUBLE_PRECISION ");
        ...
        const string& compileFlags)
{
    ...
    cl_uint numDimensions = 0;
    clGetDeviceInfo (dev[0], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                             sizeof(cl_uint), &numDimensions, NULL);

    cl_uint numDimensions2 = 0;
    clGetDeviceInfo (dev[1], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                             sizeof(cl_uint), &numDimensions2, NULL);
    numDimensions = std::min(numDimensions,numDimensions2);	

    size_t *maxWorkSizes = new size_t[numDimensions];
    clGetDeviceInfo (dev[0], CL_DEVICE_MAX_WORK_ITEM_SIZES,
                       sizeof(size_t)*numDimensions, maxWorkSizes, NULL);

    size_t *maxWorkSizes2 = new size_t[numDimensions];
    clGetDeviceInfo (dev[1], CL_DEVICE_MAX_WORK_ITEM_SIZES,
                       sizeof(size_t)*numDimensions, maxWorkSizes2, NULL);

    if ( numDimensions<2 || maxWorkSizes[0]<16 || maxWorkSizes[1] < 4 || maxWorkSizes2[0]<16 || maxWorkSizes2[1] < 4 )
    {
        cout << "SGEMM needs a 2-dimensional work group size of at least {16,4}." << endl;
        int passes = op.getOptionInt("passes");
        char atts[1024] = "GSize_Not_Supported";
    ...
    if (err != CL_SUCCESS) {
        char log[5000];
        size_t retsize = 0;
        err =  clGetProgramBuildInfo (prog, dev[0], CL_PROGRAM_BUILD_LOG,
                5000*sizeof(char),  log, &retsize);
    ...
    if (true) // pinned
    {
        Aobj = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
			       sizeof(T)*N*N, NULL, &err);
        CL_CHECK_ERROR(err);
        A =(T*)clEnqueueMapBuffer(queue[1],Aobj,true,CL_MAP_READ|CL_MAP_WRITE,
				       0,sizeof(T)*N*N,0, NULL,NULL,&err);
        CL_CHECK_ERROR(err);
        ...
        Bobj = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
			       sizeof(T)*N*N, NULL, &err);
        CL_CHECK_ERROR(err);
        B =(T*)clEnqueueMapBuffer(queue[1],Bobj,true,CL_MAP_READ|CL_MAP_WRITE,
				       0,sizeof(T)*N*N,0, NULL,NULL,&err);
        CL_CHECK_ERROR(err);
        ...
        Cobj = clCreateBuffer(ctx, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR,
			       sizeof(T)*N*N, NULL, &err);
        CL_CHECK_ERROR(err);
        C =(T*)clEnqueueMapBuffer(queue[1],Cobj,true,CL_MAP_READ|CL_MAP_WRITE,
				       0,sizeof(T)*N*N,0, NULL,NULL,&err);
        CL_CHECK_ERROR(err);
    }
    else
    ...
    // Pass A and B to the GPU and create a GPU buffer for C
    cl_mem Agpu = clCreateBuffer(ctx, CL_MEM_READ_WRITE| CL_MEM_ALLOC_HOST_PTR,
                                 m*k * sizeof(T), NULL, &err);
    CL_BAIL_ON_ERROR(err);
    cl_mem Bgpu = clCreateBuffer(ctx, CL_MEM_READ_WRITE| CL_MEM_ALLOC_HOST_PTR,
                                 k*n * sizeof(T), NULL, &err);
    CL_BAIL_ON_ERROR(err);
    cl_mem Cgpu = clCreateBuffer(ctx, CL_MEM_READ_WRITE| CL_MEM_ALLOC_HOST_PTR,
                                 m*n * sizeof(T), NULL, &err);
    CL_BAIL_ON_ERROR(err);
    ...
    for (int i = 0; i < passes; i++) {
        ...
        err = clEnqueueWriteBuffer(queue[1], Agpu, CL_TRUE, 0, m*n*sizeof(T),
                A, 0, NULL, &evDownload1.CLEvent());
        err = clEnqueueWriteBuffer(queue[1], Bgpu, CL_TRUE, 0, m*n*sizeof(T),
                B, 0, NULL, NULL);
        err = clEnqueueWriteBuffer(queue[1], Cgpu, CL_TRUE, 0, m*n*sizeof(T),
                C, 0, NULL, NULL);
        ...
        //Launch Kernels
	cl_event *evNN = (cl_event*)malloc(sizeof(cl_event)*2);

        int rt = clEnqueueNDRangeKernel_tony(queue, sgemmNN, 2, globalWorkSize,
                                     localWorkSize, evNN);
	if(rt!=-1){
    ...
    for (int i = 0; i < passes; i++) {
        ...
        Event evUpload("Upload");
//        Event evNT("sgemmNT");
	cl_event *evNT = (cl_event*)malloc(sizeof(cl_event)*2);
        ...
        //Launch Kernels
        int rt = clEnqueueNDRangeKernel_tony(queue, sgemmNT, 2, globalWorkSize,
                                     localWorkSize, evNT);
	 if(rt!=-1){
