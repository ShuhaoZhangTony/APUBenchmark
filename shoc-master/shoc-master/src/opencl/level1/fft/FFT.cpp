...
template <class T2> void runTest_ext(const string& name, cl_device_id* id,
...
RunBenchmark_ext(cl_device_id *dev,
                  ...
                  OptionParser &op)
{
    if (getMaxWorkGroupSize(dev[0]) < 64 || getMaxWorkGroupSize(dev[1]) < 64  ) {
        cout << "FFT requires MaxWorkGroupSize of at least 64" << endl;
        fillResultDB("SP-FFT", "MaxWorkGroupSize<64", op, resultDB);
        fillResultDB("DP-FFT", "MaxWorkGroupSize<64", op, resultDB);
        return;
    ...
    bool has_dp = (checkExtension(dev[0], "cl_khr_fp64") && checkExtension(dev[1], "cl_khr_fp64")) || (checkExtension(dev[0], "cl_amd_fp64") && checkExtension(dev[1], "cl_amd_fp64"));

    if (op.getOptionBool("dump-sp")) {
        dump<cplxflt>(op, dev[0], ctx, queue[1]);
    }
    else if (op.getOptionBool("dump-dp")) {
        if (!has_dp) {
            cout << "dump-dp: no double precision support!\n";
        ...
        dump<cplxdbl>(op, dev[0], ctx, queue[1]);
    }
    else {
        // Always run single precision test
...
template <class T2>
void runTest_ext(const string& name,
             cl_device_id* id,
             cl_context ctx,
             cl_command_queue* queue,
             ResultDatabase &resultDB,
             OptionParser &op)
{
    ...
    init_ext(op, do_dp, id, ctx, queue, fftProg, fftKrnl,
        ifftKrnl, chkKrnl);
