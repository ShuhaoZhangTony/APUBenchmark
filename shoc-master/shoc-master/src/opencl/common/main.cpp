...
void RunBenchmark_ext(cl_device_id *dev,
                  cl_context ctx,
...
int main(int argc, char *argv[])
{
    ...
    try
    {
        ...
        OptionParser op;
        op.addOption("tony_device",OPT_INT,"0","Limit the use of CPU and GPU");
	op.addOption("gpu_offset",OPT_INT,"50","percentage of total work put on GPU"); 
...
#define DEBUG_DEVICE_CONTAINER 0
   	...
   	cl_device_id * DevId =(cl_device_id*)malloc(sizeof(cl_device_id)*2);
        DevId[0]= ListDevicesAndGetDevice(platform, device);
        DevId[1]= ListDevicesAndGetDevice(platform, device+1);
/*	cout<<id.getInfo<CL_DEVICE_TYPE>()<<endl;
	cout<<id2.getInfo<CL_DEVICE_TYPE>()<<endl;
...
*/
	cl_int clErr;
        ...
        cl_command_queue *queue=(cl_command_queue*)malloc(sizeof(cl_command_queue)*2);
	
	queue[0] = clCreateCommandQueue( ctx,
                                                        DevId[0],
                                                        CL_QUEUE_PROFILING_ENABLE,
                                                        &clErr );
	queue[1] = clCreateCommandQueue( ctx,
                                                        DevId[1],
                                                        CL_QUEUE_PROFILING_ENABLE,
                                                        &clErr );
        CL_CHECK_ERROR(clErr);
