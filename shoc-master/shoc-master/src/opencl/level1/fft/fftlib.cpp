...
cl_ulong getSubmitEndRuntime()
{
	return std::max(SubmitEndRuntime(0),SubmitEndRuntime(1));
}
void
init_ext(OptionParser& op,
     bool do_dp,
     cl_device_id* fftDev,
     cl_context fftCtx,
     cl_command_queue* fftQueue,
     ...
     cl_kernel& chkKrnl)
{
    ...
    else if (checkExtension(fftDev[0], "cl_khr_fp64")&& checkExtension(fftDev[1], "cl_khr_fp64")) {
        args += " -DK_DOUBLE_PRECISION ";
    }
    else if (checkExtension(fftDev[0], "cl_amd_fp64") && checkExtension(fftDev[1], "cl_amd_fp64")) {
        args += " -DAMD_DOUBLE_PRECISION ";
    }
    err = clBuildProgram(fftProg, 0, NULL, args.c_str(), NULL, NULL); // compile for all.
    {
        char* log = NULL;
        size_t bytesRequired = 0;
        err = clGetProgramBuildInfo(fftProg,
    ...
    if (err != CL_SUCCESS) {
        char log[50000];
        size_t retsize = 0;
        err = clGetProgramBuildInfo(fftProg, fftDev[1], CL_PROGRAM_BUILD_LOG,
                                    50000*sizeof(char),  log, &retsize);
        CL_CHECK_ERROR(err);
        cout << "Retsize: " << retsize << endl;
        cout << "Log: " << log << endl;
...
void deinit_ext(cl_command_queue* fftQueue,
            cl_program& fftProg,
            cl_kernel& fftKrnl,
            cl_kernel& ifftKrnl,
            cl_kernel& chkKrnl)
{
    for (map<void*, cl_mem>::iterator it = memobjmap.begin(); it != memobjmap.end(); ++it) {
        clEnqueueUnmapMemObject(fftQueue[0], it->second, it->first, 0, NULL, NULL);
        clEnqueueUnmapMemObject(fftQueue[1], it->second, it->first, 0, NULL, NULL);
        clReleaseMemObject(it->second);
    }
         ...
         cl_command_queue* fftQueue)
{
    ...
    clSetKernelArg(fftKrnl, 0, sizeof(cl_mem), workp);
    int rt = clEnqueueNDRangeKernel_tony(fftQueue, fftKrnl, 1,
                       ...
                       cl_command_queue fftQueue)
{
    ...
    **(cl_mem**)bufferp = clCreateBuffer(fftCtx, CL_MEM_READ_WRITE|CL_MEM_ALLOC_HOST_PTR, bytes,
                                      NULL, &err);
    CL_CHECK_ERROR(err);
}
